...
scene_list=['정상 시나리오','아이디 입력란 공백','PW 입력란 공백','PW 확인란 공백','닉네임 공백','이름 공백','학번 공백','이메일 공백','핸드폰 번호 공백','체크박스 미체크'] #시나리오 리스트
...
exl_sheet.column_dimensions['G'].width=50
...
def signup_non_check(scene : int): #구분 값
    now = datetime.now()
    expected_val = '회원가입 실패 알림창 등장'
    inform = [scene, scene_list[scene], str(test_data), expected_val, now, str(False)]  # 테스트 수행 정보
